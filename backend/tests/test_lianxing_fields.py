...
def test_field_value_supports_nested_data_path() -> None:
    ...
    assert _field_value(record, "gross_profit") == "12.5"
    assert _field_value(record, "data.sids") == [110]
    assert _store_value(record, "data>>sids") == "110"


def test_field_value_supports_unwrapped_data_path() -> None:
    record = {"sids": [110], "gross_profit": "12.5"}
...
def test_fetch_erp_source_values_sums_nested_data_arrays() -> None:
    class FakeClient:
        def fetch_source_records(self, source, start_date, end_date, granularity):
            return [
                {
                    "_request_period": "2026-05-01",
                    "data": [
                        {"sids": [110], "gross_profit": "12.5", "amount": "100"},
                        {"sids": [110], "gross_profit": "7.5", "amount": "50"},
                    ],
                }
            ]
    ...
    source = Source(
        ...
        period_mode="request_month",
        metrics=[
            Metric(name="毛利润", warehouse_expression="gross_profit", erp_field="data>>gross_profit"),
            Metric(name="销售额", warehouse_expression="amount", erp_field="data>>amount"),
        ],
    )

    assert fetch_erp_source_values(source, "2026-05-01", "2026-05-31", "month", client=FakeClient()) == {
        ("2026-05-01", "110", "毛利润", "ERP"): Decimal("20.0"),
        ("2026-05-01", "110", "销售额", "ERP"): Decimal("150"),
    }
